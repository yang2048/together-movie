...
(async function () {
  ...
  // 配置
  (function injectSettingsScript() {
    const settingsScript = document.createElement('script')
    settingsScript.type = 'text/javascript'
    settingsScript.text = `
      window.videoTogetherWebsiteSettingUrl = "https://setting.2gether.video/v2_website.html";
    `;
  ...
  if (type == 'Chrome' || type == 'Safari' || type == 'Firefox') {
    getGM().setValue = async (key, value) => {
      return await new Promise((resolve, reject) => {
        try {
          let item = {}
          item[key] = value
          getBrowser().storage.local.set(item, () => {
            resolve()
          })
        }
        catch (e) {
    ...
    getGM().getValue = async (key) => {
      return await new Promise((resolve, reject) => {
        try {
          getBrowser().storage.local.get([key], (result) => {
            resolve(result[key])
          })
        }
        catch (e) {
    ...
    getGM().getTab = async () => {
      return await new Promise((resolve, reject) => {
        try {
          getBrowser().runtime.sendMessage(JSON.stringify({ type: 1 }), (response) => {
            resolve(response)
          })
        }
        catch (e) {
    ...
    getGM().saveTab = async (tab) => {
      return await new Promise((resolve, reject) => {
        try {
          getBrowser().runtime.sendMessage(JSON.stringify({ type: 2, tab }), (response) => {
            resolve(response)
          })
        }
        catch (e) {
    ...
    getGM().xmlHttpRequest = async (props) => {
      try {
        getBrowser().runtime.sendMessage(JSON.stringify({ type: 3, props }), (response) => {
  ...
  try {
    ...
    if (cachedVersion == vtRefreshVersion) {
      ...
      fetch(getCdnPath(encodeFastlyJsdelivrCdn, `release/vt.${language}.${vtType}.js?vtRefreshVersion=${vtRefreshVersion}`))
        ...
        .catch(() => {
          getChinaCdnB().then(chinaCdnB => fetch(getCdnPath(chinaCdnB, `release/vt.${language}.${vtType}.js?vtRefreshVersion=${vtRefreshVersion}`))).then(r => r.text()).then(data => getGM().setValue('PrivateCachedVt', {
            version: vtRefreshVersion,
            data,
          }))
        })
...
`;
  ...
  switch (type) {
      ...
      document.addEventListener('securitypolicyviolation', (e) => {
        ...
        for (let blockedStr of [e.blockedURI, e.sample]) {
          for (let extensionUrl of ['2gether.video', 'jsdelivr.net']) {
            try {
              if (blockedStr.includes(extensionUrl)) {
                urlDisabled = true
              }
            }
            catch { }
    ...
    case 'userscript_debug':
      script.src = `http://127.0.0.1:7000/release/vt.debug.${language}.user.js?timestamp=${Number.parseInt(Date.now())}`
      break
    case 'website':
      script.src = getCdnPath(encodeFastlyJsdelivrCdn, `release/vt.${language}.website.js?timestamp=${version}`)
      break
    case 'website_debug':
      script.src = `http://127.0.0.1:7000/release/vt.debug.${language}.website.js?timestamp=${Number.parseInt(Date.now())}`
      break
  }
